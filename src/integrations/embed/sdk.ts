/**
 * geojson.app Embed SDK — the host-page side of the embed protocol.
 * Built standalone as `/embed.js` (IIFE, no dependencies).
 *
 * Usage — one-shot:
...
function createEmbed(options: EmbedOptions): EmbedInstance {
  ...
  const instance: EmbedInstance = {
    ...
    listLayers: () => controller.call<LayerInfo[]>('listLayers'),
    setLayerVisibility: (id, visible) =>
      controller.call('setLayerVisibility', { id, visible }).then(() => undefined),
