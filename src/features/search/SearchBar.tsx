...
const SEARCH_LAYER_NAME = 'Search results';
...
function photonToGeoJsonFeature(pf: PhotonFeature): Feature {
...
export default function SearchBar() {
  ...
  // when a new result is picked.
  const activeSearchIdRef = useRef<string | null>(null);

  const showDropdown = isOpen && (results.length > 0 || isLoading);

  // The user can delete the pinned search feature via context menu — forget it.
  useEffect(() => {
    const id = activeSearchIdRef.current;
    if (id && !findFeature(layers, id)) {
  ...
  const selectResult = useCallback(
    (feature: PhotonFeature) => {
      ...
      if (activeSearchIdRef.current) {
        store.removeFeature(activeSearchIdRef.current);
        activeSearchIdRef.current = null;
      }

      const geoFeature = photonToGeoJsonFeature(feature);
      const fid = store.addFeature(geoFeature, {
        layerName: SEARCH_LAYER_NAME,
        origin: 'search',
      });
      if (fid) {
        store.selectFeature(fid);
        useUiStore.getState().requestFocus({ kind: 'feature', featureId: fid });
        activeSearchIdRef.current = fid;
      }
